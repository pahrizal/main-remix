...
const GameScreen = () => {
    ...
    const currentPlayer = useSelector((state: AppState) => state.game.currentPlayer);
    ...
    const winner = useSelector((state: AppState) => state.game.winner);
    ...
    React.useEffect(() => {
        if (data && data.id && socket) {
            ...
            if (localData) {
                ...
                if (data.status === GameStatus.WAITING && data.playerCount && data.playerCount < 4) {
    ...
    return (
        <div className="w-screen h-screen flex flex-col justify-center items-center">
            ...
            {winner && <GameResult {...winner} isTheWinner={winner.id === dataState?.playerData.id} />}
        </div>
    );
};
